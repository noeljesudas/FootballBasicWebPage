...
export default function HomePage() {
  ...
  const [showTrends, setShowTrends] = useState(false); // State to toggle the trends view
  ...
  const mockData = {
    ...
    VisionPro: {
      foulDetectionAccuracy: "98%",
      decisionSpeed: "3.5 seconds",
    },
  };
    ...
    <main className="min-h-screen text-white font-sans overflow-hidden">
      ...
      >
        ...
        >
          ...
          >
            <Button
              className="mt-8 px-6 py-3 text-lg font-semibold rounded-full bg-gradient-to-r from-green-400 to-blue-600 shadow-lg hover:scale-105 transition-transform duration-300"
              onClick={() => setShowTrends(true)} // Toggle trends on click
            >
      ...
      >
        ...
        <div className="grid md:grid-cols-2 gap-12 max-w-6xl mx-auto">
          {[{ title: "MAANVI", desc: "Precision match and player analytics for Tier 1 and lower leagues.", img: "/maanvi.jpg" }, { title: "VISION Pro", desc: "Real-time foul and decision detection bridging flaws of traditional VAR.", img: "/VisionPro.jpg" }].map((model, index) => (
            <motion.div
              key={model.title}
      ...
      {/* Display Real-Time Trends for MAANVI & Vision Pro when "Explore Our Solutions" is clicked */}
      {showTrends && (
        <section id="trends" className="py-24 px-6 bg-black text-center">
          <motion.h2
          ...
          <div className="grid md:grid-cols-2 gap-12 max-w-6xl mx-auto">
            <div className="bg-gradient-to-br from-green-400 to-blue-600 p-8 rounded-xl text-white">
              <h3 className="text-2xl font-semibold mb-4">MAANVI Trends</h3>
              <p><strong>Match Performance:</strong> {mockData.Maanvi.matchPerformance}</p>
              <p><strong>Player Analytics:</strong> {mockData.Maanvi.playerAnalytics}</p>
            </div>
            <div className="bg-gradient-to-br from-red-500 to-yellow-500 p-8 rounded-xl text-white">
              <h3 className="text-2xl font-semibold mb-4">VISION Pro Trends</h3>
              <p><strong>Foul Detection Accuracy:</strong> {mockData.VisionPro.foulDetectionAccuracy}</p>
              <p><strong>Decision Speed:</strong> {mockData.VisionPro.decisionSpeed}</p>
            </div>
          </div>
        </section>
      )}
...
The future of football is data-driven — and we’re here to lead the way.
      ...
      {showModal && (
        ...
        <p className="text-white/70 max-w-xl mx-auto mb-8 text-lg">
          Have questions, feedback, or just want to collaborate with us? Reach out to Drishti Technologies!
        </p>
      ...
      {/* Footer */}
      <footer className="py-8 bg-gradient-to-br from-black to-gray-800 text-center">
        <p className="text-white text-sm">
          &copy; {new Date().getFullYear()} Drishti Technologies. All rights reserved.
        </p>
      </footer>
    </main>
  );
